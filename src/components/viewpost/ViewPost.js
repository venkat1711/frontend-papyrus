import React, { Fragment,Component } from 'react';
import { withRouter, Link } from "react-router-dom";
import { Container, Row, Col, Card, InputGroup, FormControl, Button, Table } from "react-bootstrap";
import { isAuthenticated } from '../../api/auth';
import { getSingleRegisterPost ,getSinglePost} from '../../api/AllPosts/allposts';
import ImageComponent from '../dashboard/imageComponent/image';
import { toast } from 'react-toastify';
import './css/ViewPost.css';
import {API} from '../../config'

export default class ViewPost extends Component {
  constructor(props){
    super(props);
    this.state = {
      data: null
    }
  }
  getViewPostData(){
    getSinglePost(this.props.match.params.id).then((res) => {
      if (res.error) {
        toast.error(`get pagecontent error`);
      } else {
         
        
          this.setState({
           data:res
           });
            console.log(res.error);
      }
  }) .catch((err) => console.log(err, 'error in create post'));
}

  componentWillMount () {
    var data = this.getViewPostData();
    this.setState({data : data});
}


  
   

  

  bookFormRender(name){
   
    if(name=='roll'){
    return   (<React.Fragment> <tr>
                        <th> number of columns</th>
                        <td>{this.state.data.columns === undefined ? '' : this.state.data.columns}</td>
                      </tr>
                      <tr>
                        <th>Upper Margin</th>
                        <td>{this.state.data.upperMargin === undefined ? '' : this.state.data.upperMargin}</td>
                      </tr>
                      <tr>
                        <th>Lower Margin</th>
                        <td>{this.state.data.lowerMargin === undefined ? '' : this.state.data.lowerMargin}</td>
                      </tr>
                      <tr>
                        <th> Inter Column space</th>
                        <td>{this.state.data.intercolumnspace === undefined ? '' : this.state.data.intercolumnspace}</td>
                      </tr><tr>
                        <th>Lines per column</th>
                        <td>{this.state.data.linespercolumn === undefined ? '' : this.state.data.linespercolumn}</td>
                      </tr>
                      <tr>
                        <th> Dimensions</th>
                        <td>{this.state.data.dimension === undefined ? '' : this.state.data.dimension}</td>
                      </tr></React.Fragment>)
     }
     else if(name=='codex'){
      return   (<React.Fragment> <tr>
        <th> number of columns</th>
        <td>{this.state.data.columns === undefined ? '' : this.state.data.columns}</td>
      </tr>
      <tr>
        <th>Upper Margin</th>
        <td>{this.state.data.upperMargin === undefined ? '' : this.state.data.upperMargin}</td>
      </tr>
      <tr>
        <th>Lower Margin</th>
        <td>{this.state.data.lowerMargin === undefined ? '' : this.state.data.lowerMargin}</td>
      </tr>
      <tr>
        <th> Number of folios</th>
        <td>{this.state.data.numberoffolios === undefined ? '' : this.state.data.numberoffolios}</td>
      </tr><tr>
        <th>Lines per column</th>
        <td>{this.state.data.linespercolumn === undefined ? '' : this.state.data.linespercolumn}</td>
      </tr>
      <tr>
        <th> Dimensions</th>
        <td>{this.state.data.dimension === undefined ? '' : this.state.data.dimension}</td>
      </tr><tr>
        <th>Inner Marging</th>
        <td>{this.state.data.innerMargin === undefined ? '' : this.state.data.innerMargin}</td>
      </tr>
      <tr>
        <th> External Margin</th>
        <td>{this.state.data.externalMargin === undefined ? '' : this.state.data.externalMargin}</td>
      </tr></React.Fragment>)
    }else if(name=='sheet'){
      return   (<React.Fragment> <tr>
        <th> number of columns</th>
        <td>{this.state.data.columns === undefined ? '' : this.state.data.columns}</td>
      </tr>
      <tr>
        <th>Upper Margin</th>
        <td>{this.state.data.upperMargin === undefined ? '' :this.state.data.upperMargin}</td>
      </tr>
      <tr>
        <th>Lower Margin</th>
        <td>{this.state.data.lowerMargin === undefined ? '' : this.state.data.lowerMargin}</td>
      </tr>
      <tr>
        <th> Inter Column space</th>
        <td>{this.state.data.intercolumnspace === undefined ? '' : this.state.data.intercolumnspace}</td>
      </tr><tr>
        <th>Lines per column</th>
        <td>{this.state.data.linespercolumn === undefined ? '' : this.state.data.linespercolumn}</td>
      </tr>
      <tr>
        <th> Dimensions</th>
        <td>{this.state.data.dimension === undefined ? '' : this.state.data.dimension}</td>
      </tr> <tr>
        <th>Lateral Margin</th>
        <td>{this.state.data.lateralMargin === undefined ? '' : this.state.data.lateralMargin}</td>
      </tr></React.Fragment>)
    }else if(this.state.data.bookform=='tablet'){
      return (<React.Fragment><tr>
      <th>Number</th>
      <td>{this.state.data.number === undefined ? '' : this.state.data.number}</td>
    </tr>
    <tr>
      <th> Dimensions</th>
      <td>{this.state.data.dimension === undefined ? '' : this.state.data.dimension}</td>
    </tr></React.Fragment>)
    }else{
      return (<React.Fragment>
    <tr>
      <th> Dimensions</th>
      <td>{this.state.data.dimension === undefined ? '' : this.state.data.dimension}</td>
    </tr></React.Fragment>)
    }
  };
  render() {
    const download = e => {
      console.log(e.target.href);
      fetch(e.target.href, {
        method: "GET",
        headers: {}
      })
        .then(response => {
          response.arrayBuffer().then(function(buffer) {
            const url = window.URL.createObjectURL(new Blob([buffer]));
            const link = document.createElement("a");
            link.href = url;
            link.setAttribute("download", "image.png"); //or any other extension
            document.body.appendChild(link);
            link.click();
          });
        })
        .catch(err => {
          console.log(err);
        });
      }
    if(this.state.data==null){
      return <div>loading....</div>
    }
  return (
    <Fragment>
      <Container className=" mt-3">
        <Fragment>
          <Row >
            <Col md={8} className='mt-4'>
              <Card>
                <Table responsive="sm" >
                  <tbody>
                    <tr>
                      <th>Papyrus ID</th>
                      <td>{this.state.data.papyrusId === undefined ? '' : this.state.data.papyrusId}</td>
                    </tr>
                    <tr>
                      <th>Inventory,Number</th>
                      <td>{this.state.data.inventory === undefined ? '' : this.state.data.inventory},
                      {this.state.data.inventoryNumber === undefined ? '' : this.state.data.inventoryNumber}</td>
                    </tr>
                     <tr>
                      <th>Editions</th>
                      <td>{this.state.data.editionName === undefined ? '' : this.state.data.editionName}</td>
                    </tr>
                    <tr>
                      <th>TM</th>
                      <td>{this.state.data.TM === undefined ? '' : this.state.data.TM}</td>
                    </tr>
                    <tr>
                      <th>MP3</th>
                      <td>{this.state.data.MP3 === undefined ? '' : this.state.data.MP3}</td>
                    </tr>
                    <tr>
                      <th>Find place</th>
                      <td>{this.state.data.provenance === undefined ? '' : this.state.data.provenance}</td>
                    </tr>
                    <tr>
                      <th>Date</th>
                      <td>{this.state.dataeditiondata === undefined ? '' : this.state.dataeditiondata}</td>
                    </tr>
                    <tr>
                      <th>Notes to date</th>
                      <td>{this.state.data.noteDate === undefined ? '' : this.state.data.noteDate}</td>
                    </tr>
                    <tr>
                      <th>Acquisition</th>
                      <td>{this.state.data.acquisition === undefined ? '' : this.state.data.acquisition}</td>
                    </tr>
 
                    <tr>
                      <th>Notes on Provenance and Acquisition</th>
                      <td>{this.state.data.noteONPA === undefined ? '' : this.state.data.noteONPA}</td>
                    </tr>
                    <tr>
                      <th>Script Description</th>
                      <td>{this.state.data.scriptDescription === undefined ? '' : this.state.data.scriptDescription}</td>
                    </tr>
                    <tr>
                      <th>Philological Features</th>
                      <td>{this.state.data.philologicalFeatures === undefined ? '' : this.state.data.philologicalFeatures}</td>
                    </tr>
                    <tr>
                      <th>Paratextual Features</th>
                      <td>{this.state.data.paratextualFeatures === undefined ? '' : this.state.data.paratextualFeatures}</td>
                    </tr>
                    <tr>
                      <th>Note</th>
                      <td>{this.state.data.note === undefined ? '' : this.state.data.note}</td>
                    </tr>
                    <tr>
                      <th>Bibliography</th>
                      <td>{this.state.data.bibliography === undefined ? '' : this.state.data.bibliography}</td>
                    </tr>
                    <tr>
                      <th>PN</th>
                      <td><a href={this.state.data.PN === undefined ? '' : this.state.data.PN} 
                      target="_blank">{this.state.data.PN === undefined ? '' : this.state.data.PN}</a></td>
                    </tr>
                  </tbody>
                </Table>
              </Card>
            </Col>
            <Col md={4} >
              <div className='my-4'>
                {/* <ImageComponent item={this.state.data._id} download onClick={e => download(e)}/> */}
                <img  src={`${API}/allposts/photo/${this.state.data._id}`}  download onClick={e => download(e)}/>
              </div>
              <div>
                <Card>
                  <Table responsive="sm" >
                    <tbody>
                      <tr>
                        <th>Recto/Verso</th>
                        <td>{this.state.data.recto === undefined ? '' : this.state.data.recto}</td>
                      </tr>
                      <tr>
                        <th>Reused</th>
                        <td>{this.state.data.reused === undefined ? '' : this.state.data.reused}</td>
                      </tr>
                      <tr>
                        <th>Dimension</th>
                        <td>{this.state.data.dimension === undefined ? '' : this.state.data.dimension}</td>
                      </tr>

                      <tr>
                        <th>Columns</th>
                        <td>{this.state.data.columns === undefined ? '' : this.state.data.columns}</td>
                      </tr>
                      <tr>
                        <th>Upper Margin</th>
                        <td>{this.state.data.upperMargin === undefined ? '' : this.state.data.upperMargin}</td>
                      </tr>
                      <tr>
                        <th>Lower Margin</th>
                        <td>{this.state.data.lowerMargin === undefined ? '' : this.state.data.lowerMargin}</td>
                      </tr>
                      <tr>
                        <th>Objective Elements</th>
                        <td>{this.state.data.objectiveElements === undefined ? '' : this.state.data.objectiveElements}</td>
                      </tr>
                      <tr>
                        <th>Author</th>
                        <td>{this.state.data.author === undefined ? '' : this.state.data.author}</td>
                      </tr>
                     
                      <tr>
                        <th>Genre</th>
                        <td>{this.state.data.genre === undefined ? '' : this.state.data.genre}</td>
                      </tr>
                      <tr>
                        <th>Cartonnage</th>
                        <td>{this.state.data.cartonnage === undefined ? '' : this.state.data.cartonnage}</td>
                      </tr>
                      <tr>
                        <th>Signs</th>
                        <td>{this.state.data.signs === undefined ? '' : this.state.data.signs}</td>
                      </tr>
                      <tr>
                        <th>Annotations</th>
                        <td>{this.state.data.annotations === undefined ? '' : this.state.data.annotations}</td>
                      </tr>
                      <tr>
                        <th>ArchiveDossier</th>
                        <td>{this.state.data.archiveDossier === undefined ? '' : this.state.data.archiveDossier}</td>
                      </tr>
                      <tr>
                        <th>Corrections</th>
                        <td>{this.state.data.corrections === undefined ? '' : this.state.data.corrections}</td>
                      </tr>
                      <tr>
                        <th>Possible Reconstructions</th>
                        <td>{this.state.data.possibleReconstructions === undefined ? '' : this.state.data.possibleReconstructions}</td>
                      </tr>
                      <tr>
                        <th>No of fragments</th>
                        <td>{this.state.data.fragment === undefined ? '' : `${this.state.data.fragment}fr;`}</td>
                      </tr>
                      <tr>
                      <th>Material</th>
                      <td>{this.state.data.material === undefined ? '' : this.state.data.material}</td>
                    </tr>
                      <tr>
                        <th>Book form</th>
                        <td>{this.state.data.bookform === undefined ? '' : this.state.data.bookform}</td>
                      </tr>
                      { this.bookFormRender(this.state.data.bookform) }
                    </tbody>
                  </Table>
                </Card>
              </div>

            </Col>
          </Row>
        </Fragment>

      </Container>
    </Fragment>
  )
}
}

